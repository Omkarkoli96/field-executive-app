...
## Admin login
The `/admin` page (upload data, download the visit log) is now protected
by HTTP Basic Auth — a browser username/password prompt field executives
can't get past.

- **Local dev**: defaults to `admin` / `changeme123` if you don't set
  anything. Change this before you share the app with anyone.
- **On Render**: set `ADMIN_USERNAME` and `ADMIN_PASSWORD` as environment
  variables in the Render dashboard (they're marked `sync: false` in
  `render.yaml` so they're never committed to the repo). Without them set,
  the app falls back to the same local-dev default above — set them
  before going live.

## Known limitations, honestly stated
- Field executives still log in with just a name, no password — that's
  the architecture as specified for that side. Anyone can check in as
  any name, against any Loan ID. If you need to stop executives from
  checking in under someone else's name too, that needs a login system
  for executives as well (happy to add it).
