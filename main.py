...
def require_admin(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    just by knowing/guessing the URL."""
...
def admin_page(request: Request, db: Session = Depends(get_db), _auth: bool = Depends(require_admin)):
...
def admin_upload(file: UploadFile = File(...), db: Session = Depends(get_db), _auth: bool = Depends(require_admin)):
...
def admin_clear_customers(db: Session = Depends(get_db), _auth: bool = Depends(require_admin)):
    """Deletes every customer record. Used to wipe old test/sample data
    before loading real customers. Does NOT touch the visit log — past
    visits stay on record even if the customer they refer to is cleared."""
    db.query(Customer).delete()
    db.commit()
    return RedirectResponse("/admin?ok=cleared", status_code=303)


@app.get("/admin/export")
def admin_export(db: Session = Depends(get_db), _auth: bool = Depends(require_admin)):
